  ...
  //end of location based search
    ...
    try {
      //foreach ($keyword as $key => $value) {
        ...
        $stmt = $connect->prepare("SELECT * FROM apartments a, broker b where b.broker_id = a.broker_id AND state IN $concats OR state IN $loc OR city IN $concats OR city IN $loc OR total_rooms IN $concats OR landmark IN $concats OR landmark");
        $stmt->execute();
        $data2 = $stmt->fetchAll(PDO::FETCH_ASSOC);

        $stmt2 = $connect->prepare("SELECT * FROM tenaments t, broker b where b.broker_id = t.broker_id AND state IN $concats OR state IN $loc OR city IN $concats OR city IN $loc OR total_rooms IN $concats OR landmark ");
        $stmt2->execute();
                 ...
                 echo '<div class="card card-inverse card-info mb-3" style="padding:1%;">         
                  <div class="card-block">';
                    ...
                    echo '</div>
                      ...
                      echo '<a class="btn btn-warning float-right" href="../app/appointment.php">Book Appointment</a><br><br>';
                }
              ?>              
          </div>
        </div>
